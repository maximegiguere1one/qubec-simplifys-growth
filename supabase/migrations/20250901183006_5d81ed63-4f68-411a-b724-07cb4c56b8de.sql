-- Enhanced Analytics Functions for 10x Dashboard
...
BEGIN
  ...
  SELECT 
    ...
    CASE WHEN ad.lead_count > 0
      ...
      ELSE 0
    END as conversion_rate,
    0::numeric as cost_per_lead, -- Placeholder for future cost integration
...
BEGIN
  ...
  WITH vsl_events AS (
    SELECT 
      ...
      CASE 
        ...
        WHEN EXTRACT(hour FROM fe.created_at) BETWEEN 12 AND 17 THEN 'Après-midi (12h-17h)'  
  ...
  ORDER BY 
    CASE ve.time_period
      ...
      WHEN 'Après-midi (12h-17h)' THEN 2  
...
BEGIN
  ...
  -- Mock honeypot detections for now
  honeypot_detections := 0;

  RETURN QUERY
  VALUES 
    ('Événements Totaux', total_events::numeric, 
    ...
    ('Sessions Invalides', 
     ...
     CASE WHEN invalid_sessions::numeric / NULLIF(total_events::numeric, 0) > 0.05 THEN 'Problème' ELSE 'OK' END,
     CASE WHEN invalid_sessions::numeric / NULLIF(total_events::numeric, 0) > 0.05 THEN 'Vérifier l\'implémentation' ELSE 'RAS' END),
    ('Leads sans UTM (%)', 
    ...
    ('Leads Dupliqués', duplicate_leads::numeric,
