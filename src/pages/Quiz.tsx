...
const Quiz = () => {
  ...
  const questions = [{
    ...
    options: [{
      value: "inventory",
      label: "Compter mon inventaire et suivre mes stocks",
      score: 25,
      priority: "Gestion d'inventaire"
    }, {
      value: "billing",
      label: "Faire mes factures et tenir mes livres à jour",
      score: 20,
      priority: "Facturation"
    }, {
      value: "hr",
      label: "Gérer les horaires et calculer la paie",
      score: 15,
      priority: "Gestion des employés"
    }, {
      value: "projects",
      label: "Suivre mes projets et respecter mes échéances",
      score: 20,
      priority: "Suivi de projets"
    }, {
      value: "crm",
      label: "Tenir à jour mes contacts et suivre mes clients",
      score: 25,
      priority: "Gestion clients"
    }]
  }, {
    id: 2,
    ...
    options: [{
      value: "low",
      label: "Moins de 5 heures - c'est gérable",
      score: 5
    }, {
      value: "medium",
      label: "5 à 15 heures - ça commence à peser",
      score: 15
    }, {
      value: "high",
      label: "15 à 25 heures - c'est vraiment trop",
      score: 25
    }, {
      value: "very_high",
      label: "Plus de 25 heures - j'en ai assez !",
      score: 35
    }]
  }, {
    id: 3,
    ...
    options: [{
      value: "never",
      label: "Non, mes outils actuels me conviennent",
      score: 1
    }, {
      value: "sometimes",
      label: "Oui, de temps en temps",
      score: 2
    }, {
      value: "often",
      label: "Oui, assez souvent même !",
      score: 3
    }, {
      value: "constantly",
      label: "Tout le temps ! C'est frustrant",
      score: 4
    }]
  }, {
    id: 4,
    ...
    options: [{
      value: "skeptical",
      label: "Je serais prudent, ça semble trop beau",
      score: 1
    }, {
      value: "interested",
      label: "Ça m'intéresserait vraiment",
      score: 2
    }, {
      value: "excited",
      label: "Je serais très enthousiaste !",
      score: 3
    }, {
      value: "dream",
      label: "Ce serait un rêve qui se réalise !",
      score: 4
    }]
  }, {
    id: 5,
    ...
    options: [{
      value: "automation",
      label: "Que tout se fasse automatiquement",
      score: 2,
      type: "Système d'automatisation"
    }, {
      value: "integration",
      label: "Avoir tous mes outils dans un seul endroit",
      score: 3,
      type: "Plateforme intégrée"
    }, {
      value: "custom",
      label: "Quelque chose conçu spécifiquement pour moi",
      score: 4,
      type: "Solution sur mesure complète"
    }, {
      value: "mobile",
      label: "Pouvoir tout gérer depuis mon téléphone",
      score: 3,
      type: "Application mobile personnalisée"
    }]
  }];
  ...
  const handleNext = async () => {
    ...
    if (currentStep < totalSteps - 1) {
      ...
      if (leadId) {
        try {
          await sendQuizConfirmationEmail(leadId, totalScore, totalTimeSpent, answers, diagnosticMessage, contactInfo);
          setEmailSent(true);
        } catch (error) {
  ...
  return <div className="min-h-[100dvh] bg-gradient-background py-6 sm:py-8 md:py-12">
      <div className="container mx-auto container-mobile max-w-4xl">
        {/* Header */}
        

        {/* Contact Capture or Question Card */}
        {currentStep === 0 ? <Card id="contact-form" className="p-6 sm:p-8 md:p-10 shadow-card max-w-3xl mx-auto border-2 border-primary/20">
            <div className="space-y-6">
              <div>
                <Input id="name" placeholder="Prénom et nom" value={contactInfo.name} onChange={e => setContactInfo(prev => ({
              ...prev,
              name: e.target.value
            }))} className="text-base sm:text-lg btn-touch" autoComplete="name" autoCapitalize="words" />
              </div>
              
              <div>
                <Input id="email" type="email" placeholder="Adresse email" value={contactInfo.email} onChange={e => setContactInfo(prev => ({
              ...prev,
              email: e.target.value
            }))} className="text-base sm:text-lg btn-touch" autoComplete="email" inputMode="email" autoCapitalize="none" />
              </div>
              
              <div>
                <Input id="phone" type="tel" placeholder="Numéro de téléphone" value={contactInfo.phone} onChange={e => {
              const formatted = formatPhoneNumber(e.target.value);
              if (formatted.length <= 14) {
                // Max length for formatted phone
                setContactInfo(prev => ({
                  ...prev,
                  phone: formatted
                }));
              }
            }} className="text-base sm:text-lg btn-touch" autoComplete="tel" inputMode="tel" />
              </div>
            </div>
            
            <div className="text-center mt-6">
              <Button variant="cta-large" onClick={handleContactSubmit} disabled={isSubmittingContact} className={`w-full h-14 sm:h-16 ${mobileButtonClass} btn-touch text-base sm:text-lg font-semibold ${animationClass} shadow-lg hover:shadow-xl transition-all duration-300`}>
                {isSubmittingContact ? "Un instant..." : "Commencer maintenant"}
                <ArrowRight className="w-5 h-5 ml-2" />
              </Button>
            </div>
          </Card> : <Card className="p-4 sm:p-6 md:p-8 shadow-card max-w-3xl mx-auto">
            <div className="mb-8">
              ...
              return <div key={option.value} className={`flex items-center space-x-3 p-3 sm:p-4 rounded-lg border transition-all duration-300 cursor-pointer btn-touch
                      ...
                      <Label htmlFor={option.value} className="text-base sm:text-lg cursor-pointer flex-1 leading-relaxed">
                        {option.label}
                      </Label>
                      {isCurrentSelection && showFeedback && <CheckCircle className="w-5 h-5 text-green-600 animate-scale-in" />}
                    </div>;
            })}
              </RadioGroup>
            </div>
            ...
            <div className="flex justify-between items-center">
              <Button variant="outline" onClick={handlePrevious} disabled={currentStep === 1} className={`flex items-center gap-2 btn-touch ${animationClass}`}>
                <ArrowLeft className="w-4 h-4" />
                Précédent
              </Button>

              <Button variant="cta" onClick={handleNext} disabled={!answers[currentQuestion] || isAdvancingRef.current} className={`flex items-center gap-2 px-6 sm:px-8 btn-touch ${mobileButtonClass} ${animationClass} ${isAdvancingRef.current ? 'opacity-50' : ''}`} style={{
        ...
        {currentStep > 2 && !showSurvey && <MicroSurvey surveyId="quiz_experience" question="Comment trouvez-vous ce quiz jusqu'à présent ?" options={[{
        value: 'easy',
        ...
        value: 'confusing',
        label: 'Quelques questions confuses'
      }]} onComplete={() => setShowSurvey(true)} onDismiss={() => setShowSurvey(true)} />}

        {/* Quiz Completion Dialog */}
        <QuizCompletionDialog isOpen={showCompletionDialog} onClose={() => setShowCompletionDialog(false)} diagnostic={currentDiagnostic} contactName={contactInfo.name} totalScore={Object.entries(answers).reduce((sum, [questionId, answerValue]) => {
        const question = questions[parseInt(questionId)];
        const option = question.options.find(opt => opt.value === answerValue);
        return sum + (option?.score || 0);
      }, 0)} emailSent={emailSent} emailError={emailError} />
      </div>
    </div>;
};
export default Quiz;