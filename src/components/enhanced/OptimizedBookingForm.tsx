...
export const OptimizedBookingForm = ({
  ...
  const renderSingleStepForm = () => <form onSubmit={handleSubmit} className="space-y-6">
      {/* Contact Information */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <Input id="name" value={formData.name} onChange={e => handleInputChange('name', e.target.value)} className="mt-2 h-12 text-base btn-touch border-2 border-primary/30 focus:border-primary" placeholder="Votre nom complet" required autoComplete="name" />
        </div>
        ...
        <div>
          ...
          <Input id="email" type="email" value={formData.email} onChange={e => handleInputChange('email', e.target.value)} className="mt-2 h-12 text-base btn-touch border-2 border-primary/30 focus:border-primary" placeholder="votre@email.com" required autoComplete="email" />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <Input id="phone" type="tel" value={formData.phone} onChange={e => handleInputChange('phone', e.target.value)} className="mt-2 h-12 text-base btn-touch border-2 border-primary/30 focus:border-primary" placeholder="(514) 555-1234" autoComplete="tel" />
        </div>
        ...
        <div>
          ...
          <Input id="company" value={formData.company} onChange={e => handleInputChange('company', e.target.value)} className="mt-2 h-12 text-base btn-touch border-2 border-primary/30 focus:border-primary" placeholder="Nom de votre entreprise" />
        </div>
      </div>
      ...
      <div>
        ...
        <Textarea id="challenge" value={formData.challenge} onChange={e => handleInputChange('challenge', e.target.value)} className="mt-2 min-h-[100px] text-base btn-touch border-2 border-primary/30 focus:border-primary" placeholder="Ex: Je perds trop de temps à faire mes factures et suivre mes inventaires..." />
      </div>

      {/* Submit Button */}
      <div className="pt-4">
        <Button type="submit" variant="cta-large" disabled={isSubmitting} className={`w-full h-14 sm:h-16 ${mobileButtonClass} btn-touch text-base sm:text-lg font-semibold ${animationClass} shadow-lg hover:shadow-xl transition-all duration-300`}>
  ...
  return <div className="max-w-4xl mx-auto">
      ...
      <Card className="p-6 sm:p-8 md:p-10 shadow-card border-2 border-primary/20">
        {formVariant === "multi_step" ? <MultiStepBookingForm formData={formData} handleInputChange={handleInputChange} handleSubmit={handleSubmit} isSubmitting={isSubmitting} availableDates={availableDates} timeSlots={timeSlots} currentStep={currentStep} totalSteps={totalSteps} handleNext={handleNext} handlePrevious={handlePrevious} canProceedToNext={canProceedToNext()} /> : renderSingleStepForm()}
